            ...
            <tr>
                ...
                {columns && columns.map((column: any, i: number) => (
                    <th className={classes["my-table-cell"]} key={i} onClick={() => onChangeSort(column)}>
                        <div className={`flex items-center gap-x-px ${column.className ? column.className : ""}`}>
                                {column.sorter ? sortBy.dataIndex === column.dataIndex ? <FaSortUp className="text-dark-300"/> : <FaSortDown className="text-dark-300"/> : null}
                                {column.title}
                        </div>
                    </th>
                ))}
